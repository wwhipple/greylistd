...
# Knock once (deferred)
# Knock again (too soon, deferred)
# Knock again (too soon, deferred)
# Knock again (too soon, deferred)
# Knock after waiting sufficiently long time (verified)
...
my $second_try_min = '3s';      # Wait 3 seconds to try again
my $second_try_max = '7s';      # Second try within 5 seconds
my $vfy_duration = '13s';       # Verified for 13 seconds
my $log_name  = "$dir/${fname}.log";  # Log file name
my $sock_name = "$dir/${fname}.sk"; # Unix socket file
...
ok(open(OPTINS, '>', "$optin_dir/wwhipple") != 0, "Opened $optin_dir/wwhipple directory for writing");
...
##################################
# 2 N D   K N O C K : T O O   S O O N - DEFER
# 
...
ok($line eq 'yes', "I've opted in, this is my second knock (too soon--I'm deferred)");
$host->close();
...
##################################
# 3 R D   K N O C K : T O O   S O O N - DEFER
# 
...
ok($line eq 'yes', "I've opted in, this is my third knock (too soon--I'm deferred)");
$host->close();
...
##################################
# 4 T H   K N O C K : T O O   S O O N - DEFER
# 
...
ok($line eq 'yes', "I've opted in, this is my fourth knock (too soon--I'm deferred)");
$host->close();
...
ok($line eq 'no', "Fifth knock. I waited long enough, now I'm verified");
$host->close();
