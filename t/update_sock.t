#!/usr/bin/perl -w
#
# Test case to see if inotify listens for the socket message
# UPDATE <user>
# and correctly updates its list of optin domains/users
# (Otherwise, much like inotify.t)
#
use strict;
use IO::Socket;
use IO::File;
...
#
my $user_maildir = (getpwnam 'root')[7] . "/mail";
unless(-d $user_maildir) {
    mkdir $user_maildir;
}
ok(open(OPTIN2, '>', "$user_maildir/.greylist") != 0, "Opened $user_maildir/.greylist for writing");
...
###########################################################
# Send an UPDATE message to greylistd, telling it about the
# update.
$curline = 'UPDATE root';
$host = '';
...
while (my $line = $host->getline()) { # Read what it sends back
    chomp $line;
    like($line, qr/^ok$/, "Successful update of root's greylist optins");
}
