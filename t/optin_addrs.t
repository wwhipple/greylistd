#!/usr/bin/perl -w
#
# Test case to test the return of optin_addrs (SHOW_OPTIN)
#
use strict;
use IO::Socket;
use IO::File;
...
my $optin_dir = "$dir/${fname}.optins"; # Optin addresses/domains go in this directory
...
# Write some opt-in addresses
ok(mkdir($optin_dir), "Created directory $optin_dir");
ok(open(OPTIND, '>', "$optin_dir/batman") != 0, "Opened $optin_dir/batman for writing");
...
my $curline = 'SHOW_OPTIN TOSOCK ADDRS NOH';
...
while (my $line = $host->getline()) { # Read what it sends back
    chomp $line;
    if ($line !~ m/^\s*$/) {
        like($line, qr/weldon\@whipple.org|micky\@allearsstudios.com|admin\@technoids.org/, "Found domain $line");
    }
}
...
ok(unlink($conf_name, $log_name, $sock_name, $pidf_name, "$optin_dir/batman", $data_name), "Cleaned up");
